...
rank2name = [_NATION, _PROVINCE, _CITY, _COUNTY]
rank2pos_key = [_NATION_POS, _PROVINCE_POS, _CITY_POS, _COUNTY_POS]
...
class AddrInfo:
    ...
    def __init__(self, name, adcode, longitude, latitude) -> None:
        ...
        # rank 代表行政区划级别 0:国, 1: 省 2: 市 3: 县
        if self.adcode.startswith("000000"):
            self.rank = AddrInfo.RANK_NATION
            self.adcode = adcode
        elif self.adcode.endswith("0000"):
    ...
    def belong_to(self, other):
        ...
        if other.name == "中国" and self.rank != AddrInfo.RANK_NATION:
            return True
        elif other.rank == AddrInfo.RANK_NATION:
            return False
        return self.adcode.startswith(other.adcode[:other.rank * 2])
...
class CPCA:
    def __init__(self, stop_key="([省市]|特别行政区|自治区|自治州)$"):
        self.ad_map, self.matcher = _init_data(stop_key)

    def transform(self, location_strs, index=None, pos_sensitive=False, umap={}):
        ...
        import pandas as pd
        result = pd.DataFrame(
            [self._get_one_addr(sentence, pos_sensitive, umap) for sentence in location_strs],
            index=index)

        return self.tidy_order(result, pos_sensitive)

    def transform_text_with_addrs(self, text_with_addrs, index=None, pos_sensitive=False, umap={}):
        ...
        result = pd.DataFrame(filter(lambda record: record[_ADCODE] is not None,
                                     self._extract_addrs(text_with_addrs, pos_sensitive, umap, truncate_pos=False,
                                                         new_entry_when_not_belong=True)),
                              index=index)
        return self.tidy_order(result, pos_sensitive)

    def tidy_order(self, df, pos_sensitive):
        """整理顺序,唯一作用是让列的顺序好看一些"""
        if pos_sensitive:
            return df.loc[:, (_NATION, _PROVINCE, _CITY, _COUNTY, _ADDR, _ADCODE, _PROVINCE_POS, _CITY_POS,
                              _COUNTY_POS)]
        else:
            return df.loc[:, (_NATION, _PROVINCE, _CITY, _COUNTY, _ADDR, _ADCODE)]

    def _get_one_addr(self, sentence, pos_sensitive, umap):
        return next(self._extract_addrs(sentence, pos_sensitive, umap))

    def _extract_addrs(self, sentence, pos_sensitive, umap, truncate_pos=True, new_entry_when_not_belong=False) -> dict:
        """提取出 sentence 中的所有地址"""
        # 空记录
        ...
        for match_info in self.matcher.iter(sentence):
            ...
            if cur_addr and len(cur_addr.name) > 1:
    ...
    def adcode_name(self, part_adcode: str):
        addr = self.ad_map.get(_fill_adcode(part_adcode))
        return None if addr is None else addr.name

    def update_res_by_adcode(self, res: dict, adcode: str):
        if adcode.startswith("000000"):
            res[_NATION] = self.adcode_name(adcode)
            return
        if adcode.endswith("0000"):
            res[_NATION] = self.adcode_name('000000000047')
            res[_PROVINCE] = self.adcode_name(adcode[:2])
            return
        if adcode.endswith("00"):
            res[_NATION] = self.adcode_name('000000000047')
            res[_PROVINCE] = self.adcode_name(adcode[:2])
            res[_CITY] = self.adcode_name(adcode[:4])
            return
        res[_NATION] = self.adcode_name('000000000047')
        res[_PROVINCE] = self.adcode_name(adcode[:2])
        res[_CITY] = self.adcode_name(adcode[:4])
        res[_COUNTY] = self.adcode_name(adcode[:6])
...
def empty_record(pos_sensitive: bool):
    empty = {_NATION: None, _PROVINCE: None, _CITY: None, _COUNTY: None, _ADDR: None, _ADCODE: None}
    if pos_sensitive:
        empty[_NATION_POS] = -1
